...
"""DexScreener helpers with cache + throttle."""
...
def _get_cache(path: Optional[Path]) -> tuple[Dict[str, Any], Path]:
...
def _throttle(min_interval_s: float) -> None:
    ...
    if _LAST_CALL_AT <= 0:
        _LAST_CALL_AT = now
        return
    wait_s = min_interval_s - (now - _LAST_CALL_AT)
    if wait_s <= 0:
        _LAST_CALL_AT = now
        return
    time.sleep(wait_s)
...
def dexscreener_json(
    ...
    *,
    ...
    if not url:
        return None
    cache, path = _get_cache(cache_path)
    now = time.time()
    hit = _cache_hit(cache, url, now, ttl_s)
    if hit is not None:
        return hit
    _throttle(MIN_INTERVAL_S)
...
def dexscreener_search(q: str, *, ttl_s: int = DEFAULT_TTL_S) -> List[Dict[str, Any]]:
    ...
    data = dexscreener_json(url, ttl_s=ttl_s)
    if not isinstance(data, dict):
        return []
